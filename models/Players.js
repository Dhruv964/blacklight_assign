...
export const getPlayerDetails = async (uid) => {
    try {   
      const result = await pool.query(`select uid,name,(total_score/num_scores) as avg_score,country,
      dense_rank() over(order by total_score/num_scores desc) 
      as player_rank from
      (SELECT uid, name, SUM(score) AS total_score, COUNT(*) AS num_scores, country
      FROM players 
      GROUP BY uid) as player_with_scrore where uid = ?;`, [uid]);
...
const getLeaderboard = async (startOfWeek,endOfWeek,...options) => {
    try {   
        ...
        const result = await pool.query(`select *,RANK() over (order by Score DESC) as player_rank from MOCK_DATA order by Score DESC`);
